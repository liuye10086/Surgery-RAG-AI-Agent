...
def _rows(outcome_value="explicit_cirrhosis"):
    visits = [
        ...
        },
        {
            ...
            "indicators": [{"name": "alt", "value": 23, "unit": "U/L"}],
            "visit_context": {"assay_platform": "C"},
        },
    ]
    return [
        {
            ...
            "is_synthetic": False,
            "anonymous_case_code": "CASE-ABCD-2345",
            "source_trace": {"source": "registry"},
            "outcome_source": outcome_value,
...
def test_build_profiles_start_at_third_visit_and_hash_prefix_only():
    ...
    assert result.profiles[0].feature_summary["indicators"]["alt"]["unit"] == "U/L"
    assert (
        result.profiles[0].prediction_task == "fatty_liver.pre_cirrhosis_to_progression"
    )
    ...
    assert (
        result.profiles[0].timeline_sha256
        == hashlib.sha256(
...
def test_future_outcome_does_not_change_prefix_features():
    first = build_window_profiles(
        _rows("explicit_cirrhosis"), "fatty_liver", _release()
    ).profiles[0]
    second = build_window_profiles(
        _rows("explicit_hcc"), "fatty_liver", _release()
    ).profiles[0]
...
def test_cli_disease_selector_maps_to_importer_logical_dataset():
    assert resolve_reference_dataset("fatty_liver") == (
        "fatty_liver",
        "longitudinal_300",
    )
    assert resolve_reference_dataset("ad") == ("ad", "ad_longitudinal_300")
    assert resolve_reference_dataset("longitudinal_300") == (
        "fatty_liver",
        "longitudinal_300",
    )
...
def test_imported_case_record_contract_is_grouped_without_new_metadata_fields():
    metadata = {
        ...
        "event_dates": {"cirrhosis_date": "2025-08-01"},
        "is_synthetic": False,
    ...
    grouped = _group_active_source_rows(
        rows, "fatty_liver", "longitudinal_300", _release()
    )
