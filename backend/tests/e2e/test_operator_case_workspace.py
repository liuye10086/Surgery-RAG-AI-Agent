...
def _payload(disease_id=1, disease_code="fatty_liver"):
    ...
    return {
        ...
        "visits": [{
            ...
            "indicators": [{"name": "ALT" if is_fatty else "MMSE", "value": 42 if is_fatty else 28, "unit": "U/L" if is_fatty else "分"}],
...
def test_dual_disease_catalog_and_three_visit_aggregate(browser_page, operator_tokens, disease_code):
    page = browser_page
    ...
    seed = case["visits"][0]
    visits = [
        {**seed, "visit_date": day, "visit_context": {**seed.get("visit_context", {}), "facility_name": f"E2E-{day}"}}
        for day in ("2026-03-01", "2026-01-01", "2026-02-01")
    ]
    for visit in visits:
        for server_field in ("id", "case_id", "visit_index"):
...
def test_browser_editor_loads_disease_catalog_and_null_safe_value(browser_page, operator_tokens):
    page = browser_page
    ...
    with page.expect_response(lambda response: "/operator/diseases/fatty_liver/indicators" in response.url and response.status == 200):
