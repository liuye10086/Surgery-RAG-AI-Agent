...
def test_http_to_real_worker_to_fixed_history(environment, client, monkeypatch, case_index):
    response = client(1).post(
        ...
        headers={'Idempotency-Key': str(uuid4())}, json={'report_kind': 'synthetic_numeric'},
    )
    ...
    with environment[0]() as db:
        ...
        assert report.generation_fingerprint_version == 'v3'
        ...
        assert source.generation_fingerprint_version == 'v3'
    ...
    monkeypatch.setattr(numeric, 'predict_synthetic_numeric', no_current_code)
    with environment[0]() as db:
        detail = read_owned_report(db, 1, report_id)
        assert detail.publication_status == 'published'
        ...
        assert '合成数值报告' in history.items[0].title
...
def test_cancelled_synthetic_report_exposes_verified_input_audit(environment):
    ...
    with environment[0]() as db:
        ...
        assert detail.generation_context['schema_version'] == 'synthetic_numeric_generation_context.v1'
        assert detail.input_snapshot['report_kind'] == 'synthetic_numeric'
...
@pytest.mark.parametrize('field', ['content', 'prediction_result', 'evidence_snapshot', 'report_document', 'context', 'row_owner', 'row_disease', 'row_time', 'legacy_downgrade'])
...
def test_migration_refuses_discarding_published_synthetic_identity(environment, monkeypatch):
    ...
    path = Path(__file__).resolve().parents[2] / 'alembic/versions/0028_synthetic_report_publication.py'
    ...
    with environment[0]() as db:
        ...
        with pytest.raises(RuntimeError, match='discard_identity'):
            migration.downgrade()
        assert db.get(AIReport, accepted.report_id).generation_fingerprint_version == 'v3'
