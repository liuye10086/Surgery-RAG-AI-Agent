...
REQUEST = {"report_kind": "synthetic_numeric"}
...
def environment(db, integration_engine, monkeypatch):
    ...
    for flag in ("REPORT_JOBS_ENABLED", "REPORT_JOBS_ACCEPTING", "SYNTHETIC_REPORTS_ENABLED"):
...
def test_real_admission_snapshots_both_diseases(environment):
    ...
    for i in (0, 1):
        ...
        with environment[0]() as db:
            ...
            assert report.analysis_type == "synthetic_numeric"
            assert job.status == "queued"
            assert job.generation_context["schema_version"] == "synthetic_numeric_generation_context.v1"
...
def test_same_key_concurrent_admission_and_closed_gate_replay(environment, monkeypatch):
    ...
    monkeypatch.setattr(settings, "SYNTHETIC_REPORTS_ENABLED", False)
...
@pytest.mark.parametrize("action,expected", [
    ...
    ("ordinary", "synthetic_source_required"), ("old_route", "synthetic_report_kind_required"),
    ("tampered", "synthetic_source_invalid"), ("closed", "synthetic_reports_unavailable"),
])
def test_admission_rejects_without_persisting(environment, monkeypatch, action, expected):
    ...
    with factory() as db:
        ...
        db.commit()
    if action == "closed": monkeypatch.setattr(settings, "SYNTHETIC_REPORTS_ENABLED", False)
    with pytest.raises(ReportJobError) as error:
...
def test_http_permission_legacy_and_readonly_inputs(environment, client):
    ...
    assert own.get(base + "/report-readiness?report_kind=synthetic_numeric").json()["ready"] is True
...
def test_recheck_rejects_change_between_preflight_and_insert(environment, monkeypatch):
    from app.services import synthetic_report_admission as admission

    original = admission.capture_synthetic_context
    def changed(snapshot):
        ...
        with environment[0]() as db:
            ...
            db.commit()
        return context
    monkeypatch.setattr(admission, "capture_synthetic_context", changed)
