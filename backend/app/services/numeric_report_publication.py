...
from app.schemas.numeric_report import (
    NumericEvidenceSnapshot, NumericReportDocument,
    NumericPublication, NumericReportIdentity,
)
from app.schemas.numeric_prediction import NumericInput, NumericPrediction
...
def _validate_saved_prediction(prediction, numeric, context):
    ...
    result = NumericPrediction.model_validate(_raw(prediction))
...
def validate_numeric_snapshot(snapshot, context) -> tuple[NumericGenerationContext, NumericInput]:
    """Validate pinned input facts independently of inference or a saved result."""
    context = NumericGenerationContext.model_validate(_raw(context))
    ...
    if (snapshot.get("schema_version"), snapshot.get("report_kind"), snapshot.get("canonicalization_version"),
        ...
        snapshot.get("source_binding_sha256"), _saved_input_sha256(numeric)) != (
        "numeric_report_input.v1", "numeric_prediction", "v1", "sha256",
...
def _validate_snapshot(snapshot, context, prediction):
    context, numeric = validate_numeric_snapshot(snapshot, context)
    result = _validate_saved_prediction(prediction, numeric, context)
    return context, numeric, result


def build_numeric_document(report_id, created_at, snapshot, context, prediction):
    context, numeric, prediction = _validate_snapshot(snapshot, context, prediction)
    return NumericReportDocument(
        identity=NumericReportIdentity(report_id=report_id, created_at=created_at,
...
def render_numeric_document(document):
    document = NumericReportDocument.model_validate(_raw(document))
    identity, numeric = document.identity, document.numeric_input
    disease = "阿尔茨海默病" if identity.disease_code == "ad" else "脂肪肝"
    rows = ["# 数值预测报告", "", "末次值保持基线", "",
        "末次值保持仅把锚点实测值作为两个时距的比较基线，不表示病情一定不变。", "",
        "## 匿名病例与输入锚点", "",
    ...
    for observation in sorted(numeric.packets[0].input_observations, key=lambda row: (row.measured_on, row.observation_id)):
        rows.append(f"| {observation.measured_on} | {observation.known_on} | {observation.indicator.upper()} | {observation.value:g} | {observation.unit} |")
    ...
    rows.extend(["", "## 输入与证据说明", "", f"历史状态：{history}。本基线仅使用可用的锚点值，不根据历史变化外推。",
        ...
        "## 算法说明", "", "算法：末次值保持（last_value），无拟合比较基线。",
        f"算法版本：{_text(document.generation_context.algorithm.algorithm_version)}。",
        ""])
    return "\n".join(rows)


def fingerprint_v4(snapshot, context, prediction, content, evidence, document):
    return _sha({"version": "v4", "input_snapshot_sha256": compute_input_snapshot_sha256(snapshot),
...
def build_numeric_publication(snapshot, prediction, document):
    document = NumericReportDocument.model_validate(_raw(document))
    rebuilt = build_numeric_document(document.identity.report_id, document.identity.created_at,
        snapshot, document.generation_context, prediction)
    if rebuilt != document:
        raise ValueError("synthetic_document_contract_mismatch")
    evidence = NumericEvidenceSnapshot(batch_id=document.identity.batch_id,
        ...
        source_binding_sha256=document.generation_context.source_binding_sha256).model_dump(mode="json")
    prediction = document.prediction.model_dump(mode="json")
    content = render_numeric_document(document)
    return NumericPublication(content=content, prediction_result=prediction, sources=[],
        ...
        generation_fingerprint=fingerprint_v4(snapshot, document.generation_context, prediction, content, evidence, document))


def verify_numeric_integrity(snapshot, snapshot_sha256, fingerprint, prediction, content,
    ...
    try:
        if not isinstance(snapshot, dict) or snapshot_sha256 != compute_input_snapshot_sha256(snapshot):
            return False
        rebuilt = build_numeric_publication(snapshot, prediction, document)
