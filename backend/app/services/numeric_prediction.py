...
from app.schemas.numeric_prediction import NumericAlgorithm, NumericInput, NumericPrediction
...
SOURCE_FILES = (
    ...
    "backend/app/schemas/numeric_prediction.py",
    "backend/app/services/numeric_prediction.py",
)
...
def _canonical_input(raw):
    value = NumericInput.model_validate(_payload(raw)).model_dump(mode="json")
...
def numeric_algorithm_identity():
    ...
    implementation = {
        ...
        "input_schema": NumericInput.model_json_schema(),
        "result_schema": NumericPrediction.model_json_schema(),
    }
    if _source_hashes() != _LOADED_SOURCE_HASHES:
        raise ValueError("numeric_implementation_changed")
    return NumericAlgorithm(implementation_sha256=_sha(implementation), parameters_sha256=_sha({}))


def predict_numeric(raw, *, expected_algorithm=None):
    value = _canonical_input(raw)
    algorithm = numeric_algorithm_identity()
    if expected_algorithm is not None and NumericAlgorithm.model_validate(_payload(expected_algorithm)) != algorithm:
    ...
    for packet in parsed.packets:
        ...
        predictions.append({
            ...
            "status": packet.input_status, "value": anchor.value if packet.input_status == "available" else None, "reason": packet.input_reason,
        })
    result = NumericPrediction(
...
def validate_numeric_prediction(result, raw_input, expected_algorithm):
    ...
    parsed = NumericInput.model_validate(value)
    result = NumericPrediction.model_validate(_payload(result))
    algorithm = NumericAlgorithm.model_validate(_payload(expected_algorithm))
