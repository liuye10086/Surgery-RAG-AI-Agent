...
def _normalize(value: Any, *, drop_hash_declaration: bool = False) -> Any:
    ...
    if isinstance(value, (list, tuple)):
        return [
            _normalize(item, drop_hash_declaration=drop_hash_declaration)
            for item in value
        ]
...
def verify_report_integrity(
    ...
    *,
    ...
    if generation_fingerprint_version in ("v3", "v4", "v5", "v6"):
        ...
        from app.services.numeric_report_v3 import verify_numeric_v3_integrity
        verify = (verify_numeric_v3_integrity if generation_fingerprint_version == "v6" else
                  verify_numeric_v2_integrity if generation_fingerprint_version == "v5" else
    ...
    new_document = (
        ...
        or generation_fingerprint_version == "v2"
    )
    if generation_fingerprint_version not in (None, "v1", "v2"):
        return IntegrityVerificationResult(
            "invalid", "integrity_version_unknown", None, False
        )
    if new_document:
        if (
            generation_fingerprint_version != "v2"
            or report_document is None
            or not report_document_sha256
            or not input_snapshot_sha256
            or not generation_fingerprint
            or evidence_snapshot is None
            or not evidence_sha256
        ):
            return IntegrityVerificationResult(
                "invalid", "document_integrity_fields_missing", None, False
            )
        try:
            ...
            if (
                hash_report_document(ReportDocument.model_validate(report_document))
                != report_document_sha256
            ):
                return IntegrityVerificationResult(
                    "invalid", "document_integrity_mismatch", None, False
                )
        except (ValueError, TypeError):
            return IntegrityVerificationResult(
                "invalid", "document_integrity_mismatch", None, False
            )
    if new_document and saved_sources is not _SOURCES_NOT_PROVIDED:
        projection = (
            prediction_result.get("evidence")
            if isinstance(prediction_result, dict)
            else None
        )
        if (
            not isinstance(projection, dict)
            or not isinstance(prediction_result, dict)
            or saved_sources != projection.get("sources")
        ):
            return IntegrityVerificationResult(
                "invalid", "source_projection_mismatch", None, False
            )
    evidence_valid = None
    if evidence_snapshot is not None:
        ...
        if evidence_sha256 and (
            ...
            or not isinstance(evidence_snapshot.get("integrity"), dict)
            or evidence_snapshot.get("integrity", {}).get("evidence_snapshot_sha256")
            != evidence_sha256
        ):
            evidence_valid = False
        if not evidence_valid:
            return IntegrityVerificationResult(
                "invalid", "evidence_integrity_mismatch", None, None, False
            )
    if not input_snapshot_sha256 or not generation_fingerprint:
        return IntegrityVerificationResult(
            "unverifiable",
            "legacy_missing_integrity_fields",
            None,
            None,
            evidence_valid,
        )
    if (
        ...
        return IntegrityVerificationResult(
            "invalid", "integrity_input_missing", False, False, evidence_valid
        )
    try:
        input_valid = (
            compute_input_snapshot_sha256(input_snapshot) == input_snapshot_sha256
        )
        if new_document:
            from app.services.report_publication import fingerprint_v2
            ...
            fingerprint_valid = (
                create_generation_fingerprint(
                    input_snapshot, prediction_result, content, evidence_snapshot
                )
                == generation_fingerprint
            )
    except (ValueError, TypeError, AttributeError):
        return IntegrityVerificationResult(
            "invalid", "integrity_payload_invalid", False, False, evidence_valid
        )
    if input_valid and fingerprint_valid:
        return IntegrityVerificationResult(
            "valid", "integrity_verified", True, True, evidence_valid
        )
    return IntegrityVerificationResult(
        "invalid", "integrity_mismatch", input_valid, fingerprint_valid, evidence_valid
    )
