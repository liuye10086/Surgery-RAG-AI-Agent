...
2. **来源引用**
   当你引用上下文中的具体事实、数据、诊疗建议或操作步骤时，必须在对应位置使用 `[序号]` 格式标注来源（序号即上文每条信息前方括号内的数字），例如 `[1]`。每个关键论断后尽量紧跟引用。如果同一句话需要引用多个来源，必须分别使用独立编号连续标注，例如 `[5][6]`，禁止使用 `[5, 6]`、`[5，6]`、`[5、6]`、`[5-6]` 等合并格式。不要为没有来源支撑的内容伪造引用，也不要编造超出上下文范围的序号。
...
def parse_citations(text: str, total: int = 0) -> List[int]:
    ...
    主路径是 `[5][6]`，同时兼容模型偶发输出的 `[5, 6]`、`[5，6]`、
    `[5、6]` 合并引用，避免来源卡片缺失。
    """
    ids = []
    for group in _re.findall(r"\[([0-9\s,，、;；]+)\]", text):
        ids.extend(_re.findall(r"\d+", group))
    seen = set()
    result = []
    for s in ids:
        i = int(s)
