...
from app.services.numeric_model_bundle import predict_numeric_bundle, verify_numeric_bundle_runtime
from app.services.numeric_report_evidence import retrieve_numeric_evidence
from app.services.numeric_report_narrative import generate_numeric_narrative
from app.services.numeric_report_v2 import validate_numeric_v2_snapshot, build_numeric_v2_document, build_numeric_v2_publication
from app.services.report_generation_errors import safe_code


def execute_numeric_v2_report(payload, send):
    ...
    try:
        ...
        context, numeric = validate_numeric_v2_snapshot(snapshot, payload['context'])
        send({'kind': 'phase', 'phase': phase})
        verify_numeric_bundle_runtime(context.model_bundle)
        ...
        phase = 'prediction'
        send({'kind': 'phase', 'phase': phase})
        prediction = predict_numeric_bundle(numeric, context.model_bundle)
        ...
        narrative = generate_numeric_narrative(numeric, prediction, evidence, llm_model=context.llm_model)
        audit({'kind': 'task_finished', 'phase': phase, 'task': 'report_narrative', 'result_state': 'available'})
        document = build_numeric_v2_document(payload['report_id'], datetime.fromisoformat(payload['created_at']),
            snapshot, context, prediction, evidence, narrative)
        publication = build_numeric_v2_publication(snapshot, prediction, document)
