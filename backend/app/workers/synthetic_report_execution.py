...
from app.services.synthetic_numeric_prediction import predict_synthetic_numeric
from app.services.synthetic_report_publication import build_synthetic_document, build_synthetic_publication, validate_synthetic_snapshot
from app.services.report_generation_errors import safe_code


def execute_synthetic_report(payload, send):
    phase = 'model_loading'
    sequence = 0
    ...
    try:
        ...
        context, numeric = validate_synthetic_snapshot(snapshot, payload['context'])
        send({'kind': 'phase', 'phase': phase})
        phase = 'prediction'
        send({'kind': 'phase', 'phase': phase})
        prediction = predict_synthetic_numeric(numeric, expected_algorithm=context.algorithm)
        ...
        phase = 'rendering'
        send({'kind': 'phase', 'phase': phase})
        document = build_synthetic_document(payload['report_id'], datetime.fromisoformat(payload['created_at']),
                                            snapshot, context, prediction)
        publication = build_synthetic_publication(snapshot, prediction, document)
        phase = 'persistence'
