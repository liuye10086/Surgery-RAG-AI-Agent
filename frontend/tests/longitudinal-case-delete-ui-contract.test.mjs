...
test('longitudinal case deletion is exposed as a guarded danger action', async () => {
  ...
  assert.match(view, /case-management__danger/)
  ...
  const workspace = await readFile(new URL('../src/components/operator-case/OperatorCaseWorkspace.vue', import.meta.url), 'utf8')
  assert.match(workspace, /病例已归档，请先恢复病例/)
  assert.match(workspace, /疾病已停用/)
})
...
test('deleting a longitudinal case calls the API and converges local state', async () => {
  ...
  assert.match(store, /await fetchLongitudinalCases\(caseListParams\)/)
  assert.match(store, /currentLongitudinalCase\.value = null/)
  assert.match(store, /readiness\.value = null/)
})
...
test('local case state is cleared even when the post-delete refresh fails', async () => {
  ...
  const removeCase = store.slice(
    ...
    store.indexOf('\n  function ', store.indexOf('async function removeLongitudinalCase')),
  )

  assert.match(removeCase, /try\s*{[\s\S]*await fetchLongitudinalCases/)
  assert.match(removeCase, /finally\s*{[\s\S]*startNewLongitudinalCase\(\)/)
