    ...
    <article v-for="(visit, index) in visits" :key="`${visit.visit_date}-${index}`" class="timeline__card">
      ...
      <details class="timeline__context">
        ...
        <div class="timeline__context-grid">
          ...
          <label>机构名称<input :value="visit.visit_context?.facility_name ?? ''" maxlength="200" :disabled="readonly" :aria-invalid="Boolean(contextError(index, 'facility_name')) || undefined" :aria-describedby="contextError(index, 'facility_name') ? errorId(`visits.${index}.visit_context.facility_name`) : undefined" @input="updateContext(index, 'facility_name', nullableText(($event.target as HTMLInputElement).value))" /><span v-if="contextError(index, 'facility_name')" :id="errorId(`visits.${index}.visit_context.facility_name`)" class="timeline__error" role="alert">{{ contextError(index, 'facility_name') }}</span></label>
          <label>设备名称<input :value="visit.visit_context?.device_name ?? ''" maxlength="200" :disabled="readonly" :aria-invalid="Boolean(contextError(index, 'device_name')) || undefined" :aria-describedby="contextError(index, 'device_name') ? errorId(`visits.${index}.visit_context.device_name`) : undefined" @input="updateContext(index, 'device_name', nullableText(($event.target as HTMLInputElement).value))" /><span v-if="contextError(index, 'device_name')" :id="errorId(`visits.${index}.visit_context.device_name`)" class="timeline__error" role="alert">{{ contextError(index, 'device_name') }}</span></label>
          <label>检测平台<input aria-label="检测平台" :value="visit.visit_context?.assay_platform ?? ''" maxlength="200" :disabled="readonly" :aria-invalid="Boolean(contextError(index, 'assay_platform')) || undefined" :aria-describedby="contextError(index, 'assay_platform') ? errorId(`visits.${index}.visit_context.assay_platform`) : undefined" @input="updateContext(index, 'assay_platform', nullableText(($event.target as HTMLInputElement).value))" /><span v-if="contextError(index, 'assay_platform')" :id="errorId(`visits.${index}.visit_context.assay_platform`)" class="timeline__error" role="alert">{{ contextError(index, 'assay_platform') }}</span></label>
          <label>检测方法<input :value="visit.visit_context?.method ?? ''" maxlength="200" :disabled="readonly" :aria-invalid="Boolean(contextError(index, 'method')) || undefined" :aria-describedby="contextError(index, 'method') ? errorId(`visits.${index}.visit_context.method`) : undefined" @input="updateContext(index, 'method', nullableText(($event.target as HTMLInputElement).value))" /><span v-if="contextError(index, 'method')" :id="errorId(`visits.${index}.visit_context.method`)" class="timeline__error" role="alert">{{ contextError(index, 'method') }}</span></label>
          <label>样本类型<input :value="visit.visit_context?.specimen ?? ''" maxlength="100" :disabled="readonly" :aria-invalid="Boolean(contextError(index, 'specimen')) || undefined" :aria-describedby="contextError(index, 'specimen') ? errorId(`visits.${index}.visit_context.specimen`) : undefined" @input="updateContext(index, 'specimen', nullableText(($event.target as HTMLInputElement).value))" /><span v-if="contextError(index, 'specimen')" :id="errorId(`visits.${index}.visit_context.specimen`)" class="timeline__error" role="alert">{{ contextError(index, 'specimen') }}</span></label>
