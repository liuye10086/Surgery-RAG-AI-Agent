    ...
    <article v-for="(visit, index) in visits" :key="`${visit.visit_date}-${index}`" class="timeline__card">
      ...
      <div class="timeline__indicators">
        ...
        <div v-for="(indicator, indicatorIndex) in visit.indicators" :key="indicatorIndex" class="timeline__indicator">
          <label>指标
            <select :value="indicator.name" aria-label="指标名称" :disabled="readonly || !indicatorCatalog" @change="selectIndicator(index, indicatorIndex, ($event.target as HTMLSelectElement).value)">
          ...
          <label>数值<input :value="indicator.value ?? ''" aria-label="指标值" type="number" :disabled="readonly" @input="updateIndicator(index, indicatorIndex, 'value', parseNumber(($event.target as HTMLInputElement).value))" /></label>
          <label>单位
            <select :value="indicator.unit" aria-label="指标单位" :disabled="readonly || !catalogItem(indicator.name)" @change="updateIndicator(index, indicatorIndex, 'unit', ($event.target as HTMLSelectElement).value)">
      ...
      <details class="timeline__context">
        ...
        <div class="timeline__context-grid">
          <label>检测来源
            <select :value="visit.visit_context?.source_type ?? ''" aria-label="检测来源" :disabled="readonly" @change="updateContext(index, 'source_type', (($event.target as HTMLSelectElement).value as VisitSourceType) || null)">
          ...
          <label>机构名称<input :value="visit.visit_context?.facility_name ?? ''" maxlength="200" :disabled="readonly" @input="updateContext(index, 'facility_name', nullableText(($event.target as HTMLInputElement).value))" /></label>
          <label>设备名称<input :value="visit.visit_context?.device_name ?? ''" maxlength="200" :disabled="readonly" @input="updateContext(index, 'device_name', nullableText(($event.target as HTMLInputElement).value))" /></label>
          <label>检测方法<input :value="visit.visit_context?.method ?? ''" maxlength="200" :disabled="readonly" @input="updateContext(index, 'method', nullableText(($event.target as HTMLInputElement).value))" /></label>
          <label>样本类型<input :value="visit.visit_context?.specimen ?? ''" maxlength="100" :disabled="readonly" @input="updateContext(index, 'specimen', nullableText(($event.target as HTMLInputElement).value))" /></label>
          <label v-if="indicatorCatalog?.disease_code === 'fatty_liver'">影像检查类型<input :value="visit.visit_context?.imaging_type ?? ''" maxlength="100" :disabled="readonly" @input="updateContext(index, 'imaging_type', nullableText(($event.target as HTMLInputElement).value))" /></label>
          <template v-if="indicatorCatalog?.disease_code === 'ad'">
            <label>量表版本<input :value="visit.visit_context?.scale_version ?? ''" maxlength="100" :disabled="readonly" @input="updateContext(index, 'scale_version', nullableText(($event.target as HTMLInputElement).value))" /></label>
            <label>评估语言<input :value="visit.visit_context?.assessment_language ?? ''" maxlength="50" :disabled="readonly" @input="updateContext(index, 'assessment_language', nullableText(($event.target as HTMLInputElement).value))" /></label>
            <label>受教育年限<input :value="visit.visit_context?.education_years ?? ''" type="number" min="0" max="30" :disabled="readonly" @input="updateContext(index, 'education_years', parseNumber(($event.target as HTMLInputElement).value))" /></label>
          </template>
          <label class="timeline__context-wide">治疗变化<textarea :value="visit.visit_context?.treatment_change ?? ''" maxlength="2000" :disabled="readonly" @input="updateContext(index, 'treatment_change', nullableText(($event.target as HTMLTextAreaElement).value))" /></label>
          <label class="timeline__context-wide">诊断变化<textarea :value="visit.visit_context?.diagnosis_change ?? ''" maxlength="2000" :disabled="readonly" @input="updateContext(index, 'diagnosis_change', nullableText(($event.target as HTMLTextAreaElement).value))" /></label>
        </div>
      </details>
