<template>
  <div class="action-bar">
    ...
    <button v-if="!inputReadonly" type="button" :disabled="!!readonlyReason || saving || reportGenerating || !dirty" @click="$emit('save')">{{ saving ? '保存中…' : '保存病例' }}</button>
    <button v-if="showReport" type="button" class="action-bar__report" :disabled="!!readonlyReason || saving || reportGenerating || dirty || !readiness?.ready" @click="$emit('generate-report')">{{ reportGenerating ? '生成中…' : reportLabel }}</button>
  </div>
</template>

<script setup lang="ts">
import type { OperatorCaseReportReadiness } from '@/api/operator'
withDefaults(defineProps<{ dirty: boolean; saving: boolean; readonlyReason?: string; reportGenerating?: boolean; readiness: OperatorCaseReportReadiness | null; inputReadonly?: boolean; showReport?: boolean; reportLabel?: string }>(), { reportGenerating: false, readonlyReason: '', inputReadonly: false, showReport: true, reportLabel: '生成报告' })
...
.action-bar button:focus-visible { box-shadow: 0 0 0 2px var(--bg-canvas), 0 0 0 4px var(--color-primary); }
</style>
