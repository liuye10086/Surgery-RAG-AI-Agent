...
def _load():
    path = Path(__file__).parents[1] / "prepare_standard_drafts.py"
    spec = importlib.util.spec_from_file_location("prepare_standard_drafts", path)
...
def test_execute_path_rolls_back_on_failure(monkeypatch, capsys, tmp_path):
    ...
    assert script.main(["--fatty-source", str(tmp_path / "fatty.docx"), "--ad-source", str(tmp_path / "ad.docx"), "--admin-id", "7", "--execute"]) == 2
