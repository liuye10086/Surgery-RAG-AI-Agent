...
- **v5.14 追加（整体复审六轮）**：`_excluded_patient_sets` evaluator 显式剔除（supplied − unobs − unknown）+ 确认 landmark 资格显式化（w ≥ 2 且 admin_end − w ≥ hw）+ evaluator 重叠测试。
...
计划已保存（v5.17）。执行选项：**Subagent-Driven（推荐）** 或 **Inline Execution**。按既定协作框架建议**分批执行（Task 1-3、4-6、7-10、11-14）、每批交 Codex 审查、通过后推送**。实施计划 v5.17 先交 Codex 复审。
