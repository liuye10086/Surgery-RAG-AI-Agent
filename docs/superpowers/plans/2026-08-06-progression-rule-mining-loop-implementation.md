...
def _excluded_patient_sets(patients, supplied_eval_ids, hw):
    ...
    evaluator_ids = set(supplied_eval_ids) - unobs_ids - unknown_ids
...
计划已保存（v5.14）。执行选项：**Subagent-Driven（推荐）** 或 **Inline Execution**。按既定协作框架建议**分批执行（Task 1-3、4-6、7-10、11-14）、每批交 Codex 审查、通过后推送**。实施计划 v5.14 先交 Codex 复审。
