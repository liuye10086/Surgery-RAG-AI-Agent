...
def test_excluded_sets_evaluator_overlap_removed():
    ...
    # → helper 显式剔除（evaluator_ids = supplied − unobs − unknown），只计真正的 evaluator（2）
    from scale_study import _excluded_patient_sets
...
计划已保存（v5.16）。执行选项：**Subagent-Driven（推荐）** 或 **Inline Execution**。按既定协作框架建议**分批执行（Task 1-3、4-6、7-10、11-14）、每批交 Codex 审查、通过后推送**。实施计划 v5.16 先交 Codex 复审。
