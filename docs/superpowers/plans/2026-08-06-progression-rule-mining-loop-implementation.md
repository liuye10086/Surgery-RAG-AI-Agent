...
**规格依据：** `docs/superpowers/specs/2026-08-06-progression-rule-mining-loop-design.md`（v19）。
**计划修订：** v5.30 按最终 acceptance 验收实测批准的两项口径调整（Codex 确认无异议）——① **验收 CI 门控**：`evaluate` 返回新增 `r1_ci`/`r2_ci`（完整命中的 R1/R2 规则 CI）；acceptance 的 `ci_ok` 从"所有挖回规则 CI 有限"改为**只查完整命中的 R1/R2 规则 CI 有限**（低支持杂项规则 CI 未估计是统计现实，如实报告不作门控；首次 acceptance 实测 13/20 未达标、部分失败即因此，修复后 ≥90% 通过）；② **模拟样本配置**：交集组比例 0.05→0.10（p=[0.68,0.12,0.10,0.10]），使 lead-lag 交集样本 n_inter 稳定 ≥30（旧 5% 下 seed 13 实测 29）。规格 v19 同步（§9/§10 验收 CI 口径 + §5.4 样本配置 + 修订记录）。**v5.29 按实施批次 3（Task 7-10）实测修正（多轮，含用户批准的方法论决策 + Codex 批次 3 五轮评审）——**五轮（Codex 批次 3 五轮评审，条件性通过代码）**：㉖ **Task 14 验收接入路径级 unmatched_by_group**（max(有限组)、空组 NaN 忽略——全局 unmatched_rate 会被稀释绕过"某组 >20%"门槛；计划 Task 14 验收段已改）；㉗ 计划 Task 9 正文与实现全文同步仍推迟（用户既定决策，与 Task 11-14 一致性检查一起；五轮确认代码与规格 v18 一致，仅计划正文待同步）。**四轮（Codex 批次 3 四轮评审）**：partial_hit 规则级 horizon/lag → 部分命中；unmatched 空组 NaN；规格 CI estimand 明确（候选空间固定、候选存在性忽略）。**三轮（Codex 批次 3 三轮评审）**：㉑ **numpy CI lift 折内基线**（验证折自身阳性率，非全样本基线——全样本基线在折内正例率不同时改变 estimand；`base_va = y[va].mean()`）；㉒ **重复内共识重算 support（规格 §8.1 精确落实）**：mine_rules 每 seed 对发现的规则在**完整 subset（每患者 1 行 = 合并验证折列联表）重算 support**，达标才计入共识集（替代二轮"≥2 折发现"heuristic——它会让两折偶发但合并支持不足的规则进入、让单折发现但合并达标的规则排除）；lift_median = 合并列联表 lift；`test_selection_uses_full_subset_consensus`；㉓ **partial_hit 时间字段对齐规格 §8.3**：按 indicator+op 维度匹配，**时间窗（lookback）或数值阈值未达 typed 容差 → 部分命中**（indicator+op 一致但 typed_match 失败）；无关条件 → False；`test_partial_hit_time_window_out_of_tolerance`；㉔ **全链路唯一患者一致性测试** `test_unique_patient_consistency_across_paths`（_support/_lift/_discover_frozen 在重复患者/行序变化下与 drop_duplicates 后行级严格等价）；㉕ **路径绑定 fixture** `test_unmatched_group_gates_and_analysis_set_consistency`（某组进展者无同箱对照 → unmatched_by_group=1.0 → not_estimable）。⑧ **CI 折内发现判定**（Codex P1-1）：规则须在每个**训练折**支持度 ≥ 门槛（与"重跑折内发现"等价——固定切点下发现 = 训练折门槛；全体样本达标 ≠ 每训练折达标，反例：规则 ev=6 但 3 折训练集正例 ~4 < 5 → CI 未估计 + `test_rules_ci_requires_fold_train_support`）；⑨ **支持度/事件支持按唯一患者**（Codex P1-2：Bootstrap 重采样保留重复患者行，`_support`/`_lift` 行级计数会重复计；改唯一患者口径，`test_support_unique_patient_in_resample` 断言 10 行重复患者 → (1,1)）；⑩ **发现全量不截断落实**（Codex P1-3：`_discover_frozen` 去掉 `[:discover_top_k]` 硬截断——静默丢弃通过组合会再次使 planted 不可达；`test_discover_returns_all_passing` 对照枚举计数）；⑪ **per_path 限定有匹配对照的进展者**（Codex P1-5：与 control_delta 同分析集，规格 §7.1）；⑫ **control_delta_ci**（Codex P2-1：配对差异 fd−ctrl 的患者级 Bootstrap CI，新接口键）；⑬ **mine_rules 校验 len(seeds)==n_repeats**（Codex P2-2，同 fit_and_oof）；⑭ **候选跨 seed 稳定性测试**（Codex P2-3：planted 网格条件跨 seed 恒在 + 重叠率 ≥0.4；top_m 削减的通用召回敏感性为已知风险，记录于交接）；**P1-4（unmatched_rate 分母含 neither）经验证不成立**——`_observed_progressors` 的 `g==1` 过滤已排除 neither（g 恒 NaN），实测 prog 185 人 neither 0 人；**二轮（Codex 批次 3 二轮评审）**：⑮ **发现流程内联支持度也按唯一患者**（_discover_frozen 的 ev/tot nunique——掩码复用路径原按行计数）；⑯ **折级共识**（selection 只计该 seed 内 ≥2 折发现的规则——单折偶发不计，替代"任一折发现即计数"；`test_selection_requires_fold_consensus` monkeypatch 反例）；⑰ **lead-lag 相对事件零点对齐**（per_path/order 汇总 ev−fd 提前量——绝对窗口混合被事件时点驱动；`test_relative_lead_same_for_different_event_windows` fixture 不同 ev 相同相对 lead → 中位一致；afp_after_early 判据相应反转（lead 大=更早））；⑱ **指标-路径组绑定**（control_delta 限该指标相关路径组——AFP 不含 R1-only 噪声首偏）+ **order pairs ∩ matched** + **unmatched_by_group 路径级比例**（not_estimable 门槛用 max(各组)）；⑲ **CI b = 版本化 bootstrap_b（1000）**（12 次重采样分位由极值决定不稳定）+ **_rules_bootstrap_ci numpy 内核**（每样本特征矩阵提取 + 列比较向量化 + 患者去重后行级路径——pandas 逐规则 653s/CI → 1.8 分钟）；⑳ **partial_hit 排除无关条件**（mined 每条件须匹配 planted——含无关条件 → 非真子集 → False；`test_partial_hit_rejects_unrelated_condition`）。① Task 7 实现 bug：`_first_dev_by_patient` 内部 `sigma[ind]` → `sigma`（调用方传标量，计划代码未运行暴露）；② Task 8：`_lm()` followup 24→36（锚点契约连锁）；**PLT 消融断言改 HbA1c/AFP**（PLT 基线异质性 SD 37.5 > 衰减信号，PLT_cur 水平值预测力被稀释、消融契约保留 drop_pct → 移除滞后列 AUC 微升 -0.003 是模型特性）；③ Task 9 方法论（**用户批准 top_k 全量不截断**）：planted R1/R2 在真实 SUB 的 lift 排名（240/862）——模拟器信号结构（AFP 水平为最强信号）使更特异组合系统性更强，"planted=数据最优"假设不成立 → `discover_top_k` 20→1000（发现预算语义，报告只渲染 top 20+汇总 Task 12）；④ 性能：候选掩码复用（`_cond_mask` 逐层 AND，35 候选 5.9 万组合 ~30s→~1s）、候选削减（top_m 8→4、thresholds_per_feature 3→2、网格 age[50,40]/rises[2]/drop[0.2]）、max_candidates 1万→50万（折内候选并集 10.2 万组合）；⑤ **规则 CI 等价重构**：折内候选切点随折/样本/seed 漂移使"重跑发现"永不重新发现固定阈值规则（实测 0/12）→ 批量共享重采样 + **直接支持度判定**（固定切点下与"重跑发现+规则出现"数学等价）；⑥ selection 门槛 0.5→全部重复（2 seeds 下 = 出现 ≥1 次使 seed 偶发规则泛滥 5333 条输出；全部重复符合跨重复稳定性意图，R1/R2 网格候选跨 seed 固定 ✓）；⑦ 测试修正：unique_max_lift 断言改"包含 R1+全等价"（分位切点等价阈值变体并列合法）、CI 断言改"高支持规则（ev≥30）带 CI"（低支持规则 b=12 重采样重新发现 <2 次是统计现实）、partial_hit 断言与 Interfaces"非空真子集"一致（typed 容差内全匹配 = 等价变体 → False）、test_config 网格断言同步、SUB 上的 mine_rules 重测试标 slow（pytest.ini slow 分层）。**v5.28 按 Codex 批次 2 三轮评审（2 P2）修正——① Task 4 测试代码段 import 补 `FEATURE_SCHEMA`（v5.27 同步测试函数时遗漏导入，按计划复制会 NameError；实现文件已正确导入）；② 空 oof_frame 回归测试补 `patient_id` int64 dtype 断言（三列 dtype 契约全锁定：patient_id/label int64、oof float64），实现与计划同步。**v5.27 按 Codex 批次 2 二轮评审（1 P1 + 1 P2）修正——① 空数据链端到端打通：`_empty_frame` 补全 **dtype**（patient_id/window/age/sex_male/admin_end/label int64、指标派生 float64、group object、unobservable bool——object 空列在 `sub[~sub["unobservable"]]` 布尔过滤后丢失 schema，实测 (0,0)）；`fit_and_oof` not_estimable 分支的空 `oof_frame` 保留 patient_id/label/oof schema 与 dtype（Task 11 直接读 oof_frame["label"]，原无列 DataFrame KeyError）；测试覆盖 confirmation_subset → `~unobservable` → groupby 链（`test_empty_landmarks_preserve_schema` 扩展）+ 空 oof_frame schema（`test_empty_landmarks_not_estimable_chain` 扩展）；② **计划正文同步**：Task 4 实现代码段（FEATURE_SCHEMA/_empty_frame 定义 + 两函数空分支）、Task 6 实现代码段（_META_COLS/_feat_cols + n_repeats 校验 + 空 oof_frame）、Task 4/6 测试代码段（4 个新测试全文）与实现逐字一致。**v5.26 按 Codex 批次 2 评审（1 P1 + 3 P2）修正——① features 空数据集保留完整固定列 schema（`FEATURE_SCHEMA` + `_empty_frame`；原 `pd.DataFrame([])` shape (0,0) 使下游 `lm["label"]`/`sub["unobservable"]` KeyError，不返回 not_estimable）+ `test_empty_landmarks_preserve_schema` + `test_empty_landmarks_not_estimable_chain`；② `fit_and_oof` 显式校验 `len(seeds) == n_repeats`（不一致/零重复 → ValueError，不静默少跑、不在 vstack/seeds[0] 崩溃）+ `test_n_repeats_must_match_seeds`；③ 模型特征排除 admin_end（`_META_COLS` 显式列表；规格 §6.1 只定义指标派生 + age + sex_male；单网格内常量但合并随访队列会泄漏研究设计差异）+ `test_model_features_exclude_admin_end`；④ **提交模板更正（P2-3）**：Global Constraints 与全部 Task Step 5 的 `AI-Agent: Codex / AI-Client: Codex-Desktop` → **`AI-Agent: Claude-Code / AI-Client: VS-Code`**（实现者 = Claude Code，模板原写错且批次 1/2 已沿用过；历史提交**不改写**——绝不 amend，更正关系由本说明 + 后续修复提交记录）。**v5.25 按实施批次 2（Task 4-6）实测修正——Task 4 的 `test_qualifying_uses_all_landmarks`/`test_confirmation_subset_contract` 与 Task 6 的 `_lm()` 测试参数 followup_months 24→36：24 月随访（admin_end=4、hw=2）下每患者至多 1 个合格窗口（`len(lm) > nunique` 数学上不可能）且路径组全 no_feasible → 无事件 → 全负例 → `fit_and_oof` not_estimable（v5.24 锚点契约连锁，同批次 1 coverage 测试修正模式）；36 月（admin_end=6）下 3 窗/患者、路径组可观测、可估计。批次 2 实现（features/splitters/model）无其他偏离。**v5.24 按 Codex 批次 1 四轮/五轮评审修正——四轮：① 规格 §5.1 规模退化实验可行性口径更新（"24 月随访 × 12 月视界有可行 landmark" → "不可估，admin_end−hw=2<4，显式 not_estimable（§8.2 契约，`_cell_feasible`）；36/60 月可估"），与 §5.5/§8.2/Task 11 契约一致；② 采纳 P2 建议：后续批次文档自检区分历史修订记录（允许旧口径）与现行正文（§5.3/§5.5/§8.2 必须只保留 v17 口径），作为提交前检查项；五轮：③ 规格 §5.3 Z 双重角色明确（边际分组频数仅诊断；Z 按完整归属算法限定验收候选集合、不作为跨路径重新归属依据、不单独作为风险目标——消除"Z 分组统计仅作生成器内部诊断……不作为 planted-rule 风险验收"与候选集合限制的冲突）、计划标题 v5.23→v5.24（元数据同步）。**v5.23 按 Codex 批次 1 三轮评审（2 P1 + 1 P2，文档契约）修正**——① 规格 §5.3 验收分组统一为完整归属算法（先按 Z 定候选组与指定 landmark → 在指定 landmark 按可观察条件判定 → 仅 Z 候选集合内互斥组 → neither 命中 R2 只计误报不转组；"Z 不作为跨路径重新归属依据"替代旧表述"Z 不进入验收分组"），§5.5 互斥表述与可行性不等式同步（12 月视界 24 月随访无可行锚点、36 月可行；24 月视界 24/36 无可行、60 月可行）；② 计划 Task 3 代码段 `_by_w_slice` 接口残留同步（v5.22 实现已改 `(sorted_pids, order, wins, vals, cols, pid, w)`，计划仍写旧签名会诱导后续 Task 恢复旧接口 → 重新引入全局乱序 bug）；补充"调用方每个 obs 集只构建一次 `_patient_order`"；顶部元数据 v5.18→v5.23、规格依据 v16→v17；③ 多 seed 成立率测试口径注释明确（分母排除删失/事件前确认/无可行锚点 = 可评估分母；是"观测条件成立率"非全路径组覆盖率；与 condition_not_held 分母一致）。**v5.22 按 Codex 批次 1 二轮评审（4 P1 + 2 P2）修正**——① CRN 彻底化（患者构成真正固定）：一轮仅拆分 rng_gate/rng_delta 不彻底——事件窗口改变 n_t → 下一患者主 rng 流偏移 → 锚点/删失/基线漂移（Codex 复现：n=300 seed=3 gate .1 vs .9 时 w_a 差 30 行、删失窗口差 120 行；n=30 明确非单调 [0.50,0.25,0.80,...]）；修复 = 患者级派生 rng `rng_i`（锚点/删失/neither hazard/基线/噪声全患者内）+ rng_gate 每患者固定消耗 1 个 uniform（含 z=none）+ δ 用 rng_delta 独立流 + 噪声全窗口一次性生成后按 trunc 截取（rng_i 消耗固定，观测不随 gate 变）→ 患者构成（z 之外）与 gate 完全无关、g_i(mid)=1{u_i<mid} 严格单调；新测试 `test_gate_changes_only_g_and_event`（patients 全列 + obs 公共窗口一致）、`test_gate_risk_monotone_small_samples_dense_grid`（n=30/120/300 × Codex 反例种子 × 6 点细网格）；② _by_w_slice 全局乱序：一轮按实际 window 列定位只修了块内乱序，`_row_range` 的 searchsorted 仍要求全局 patient_id 有序（Codex 复现：simulate(3000,24,12,seed=4) 全局 shuffle 后 neither 误报率 0.152→0.001）；修复 = `_patient_order`（np.argsort 稳定排序一次）+ 块内按实际 window 匹配，与输入行序无关；新测试 `test_by_w_slice_global_shuffle_invariant`（完整 DataFrame shuffle × 2 seeds 结果一致）；③ 规格 §5.3 全文统一（表头"neither（无植入条件）"、验收段 v17 口径、R2-only/R1∩R2 组构成限定 Z ∈ {r2, r1_and_r2}——neither 候选患者 R2 命中只计误报、不重新归属，两种解释不同时保留）；④ Task 11 执行契约落地：`_cell_feasible`（admin_end−hw ≥ 4）→ run_cell 前置返回 `{"records": [], "not_estimable": True, "reason": "no_feasible_path_anchor"}`、run_study 聚合/可靠性边界前显式过滤（`test_run_cell_records_full_fields` 24→36 月 + 新增 `test_cell_feasibility_and_not_estimable_unit`、`test_run_study_excludes_infeasible_unit`）；⑤ 多 seed 成立率回归 `test_conditions_hold_across_seeds`（5 seeds 各路径组 ≥0.95——Codex 复现 0.891 系其脚本分母未按可评估过滤（确认点处观测截断的删失患者计入失败），规格口径（可评估）下实测 R1/R2/both 均 ≥0.988）。**v5.21 按 Codex 批次 1 评审（5 P1 + 2 P2）修正**——① 向量化基线 SD 误改（range/6 → 0.1·range，PLT 患者间异质性 37.5→22.5）恢复 range/6 + `test_obs_baseline_distribution` 回归（窗口 0 观测方差 = 基线²+噪声²）；② `p_obs` 补 not_estimable 口径（confirm_window 非有限患者跳过，规格 §5.5）+ `test_p_obs_excludes_no_anchor_patients`；③ `_by_w_slice` 改按实际 window 列定位（不假设行偏移=窗口号；Codex 打乱 obs 复现 neither 误报率 0.162→0.0006）+ `test_by_w_slice_handles_unsorted_and_missing_windows`；④ 事件门控隔离 rng（`rng_gate` g 每患者固定消耗 1 随机数 + `rng_delta` δ 独立流 → CRN 下 r(mid)=mean(g) 严格单调，校准 bisection 收敛有保证；旧共享流 n=30/120/300 实测 0.40→0.25 非单调）+ `test_gate_risk_monotone_under_crn`；⑤ 数学修正：neither R2 噪声两窗连升概率 = 1/6 ≈ 16.7%（三个 iid 连续值严格递增的排列概率），v5.19/v5.20 误写 25%（实测 17.5% 吻合 1/6），全文注释同步；⑥ Task 11 网格标记：scale_down 的 followup_months=24（admin_end=4）在锚点约束（w_R1 ≥ w0+3 ≥ 3 > admin_end−hw=2）下路径组全 no_feasible（simulate(10k,24,12) 实测 per-rule coverage 全 0）→ Task 11 实现须将该单元显式标记 not_estimable 并从恢复率/可靠性边界统计排除（§5.5），规格 v17 同步。**v5.20 按实施第一批（Task 3）实测发现的三处缺陷 + 性能优化修正**——① `_sample_anchors` 的 w_A 下界 w0+3 过度保守：AFP 信号自 w_A 起立即生效（无需累积），PLT 只需 confirm=w_A+1 ≥ w0+3（3 窗累积 → 降 48.8% → 99.3%）；w0+3 使 12 月视界（admin_end=6）下 w0=1 的 r2/r1_and_r2 患者 hi<lo 全部 no_feasible → 校准 mean(g) 可观测样本减半（~210/10k）→ 抽样噪声 3.4% > bisection tol 0.5% → gate 偏差 4.65pp、±3pp 断言不可达；改 w_A ≥ w0+2 后样本翻倍、两视界偏差 ≤1.3pp；② r1_and_r2 的 w_r1 采样空区间不再牵连 w_A（原 `return np.nan, np.nan` 把 w_a 一并丢弃 → 80% 交集组患者 no_feasible → per_rule r1 coverage 0.67<0.80）；w_r1 对交集组仅记录字段（R1 条件在 confirm=w_A+1 检查），空区间只置 w_r1=NaN；③ Task 3 测试两处修正：`test_calibrated_latent_risk_both_horizons` 补 `_group_latent_risk` import（计划测试代码缺失，14 轮评审未运行测试未暴露）、`test_coverage_contract_and_gate_reachable` followup_months 24→36（admin_end=4 下 w_r1 ≥ w0+3 ≥ 3 > admin_end−hw=2，几何不可行 → 路径组全 no_feasible → coverage=0）；性能优化（simulate 向量化）：观测生成改每患者一次批量噪声矩阵 `rng.normal(0,1,size=(n_t,n_ind))*σ` + numpy 掩码信号叠加（与逐指标逐窗 iid 分布完全相同），`_group_latent_risk`/`_compute_coverage` 的 by_w 改 numpy 窄窗口直取（`_by_w_slice`，obs 按 patient_id/window 有序 → 行 = 患者起点+窗口，绕开 groupby/to_dict 慢路径）——simulate(30k) 13.65s→2.05s、calibrate_gates 145s(5k)→73s(10k)，本批全量测试 ~30min→4.5min（Task 11/14 Monte Carlo 受益）。**v5.19 按实施第一批（Task 1-2）实测发现的两处数值/语义缺陷修正**——① config 信号强度与锚点下界：原假设"w>=w0+2 时 PLT 较基线降 ~28% >20%"在 w0=0 时实际仅 -21.4%（基线取窗口 0,1 平均含 1 窗衰减），噪声 sd ~12pp 下最小锚点余量不足，R1 条件成立率实测 0.885<0.95（14 轮评审均未运行测试，属计划数值假设错误）；修正为 `hba1c_rise_per_window` 0.8→1.0、`plt_decline_per_window` 0.85→0.80、锚点下界 w_A/w_R1 提至 w0+3（confirm=w_A+1≥w0+4），成立率 ~0.99（K=20 seeds 稳定过线）；② neither_clean 断言语义：纯 iid 噪声下 AFP 两窗连升概率 = 1/6 ≈ 16.7%（三个 iid 连续值严格递增的排列概率；与 σ 无关，修改前实测 0.8024 吻合），"neither 参考 landmark R1、R2 均不成立 ≥95%"在数学上不可达；改为只验 R1 复合条件（噪声下 ~0.1% 命中，实测 ~0.999），R2 单条件命中如实计入 `neither_false_positive_rate` 报告（规格 §5.3 本就定义命中=误报、无门槛，Task 3 仅断言 `0≤rate≤1`）。**v4 按 Codex 计划第三轮 13 条意见修正；**v4.1 按 Workflow 对抗验证 10 条真实发现修正**；**v5**（Codex 第四轮 8 大项）；**v5.1**（Codex 第五轮 12 项）；**v5.2 按 Codex 计划第六轮 11 项修正**——R1 ≥95% 测试改按 z + 指定锚点直接验证（脱离 group/unobservable 生成判定循环）、`_group_latent_risk` 排除 neither 误报患者（obs 传参 + 校准测试同分母）、coverage 文档口径统一（excluded 单列不从规则覆盖率分母移除）、mine_rules 内部过滤 unobservable、候选网格版本化 + 确定性顺序（candidate_grid 版本化）+ synthetic fixture 测试、control_delta 对照无偏离取 cutoff 端点（有限值）+ 确定性手工 fixture、边界 Bootstrap B/有效比例版本化 + +inf CI 契约、run_cell excluded_breakdown 明细 + 字段语义修正、`_fold_discover_validate` 签名统一（显式 horizon）、acceptance ci_ok 加强（长度一致 + 有限数值区间）。**v5.3 按 Codex 计划第七轮 10 项修正**——R1 条件成立率口径限定可评估患者 + unobservable 原因分解（删失不设门槛、条件未成立 ≤5%，patients 表新增 `unobservable_reason`）、neither 误报分母改为全部 neither 候选患者（规格 §5.3 口径，校准分母不变）、`_bisect` 端点包围检查 + 上界自适应扩展（非包围显式 ValueError）、synthetic fixture 加确定性负例（折数可用）+ `_discover_frozen` 发现流程确定性（discover_top_k 版本化）、Task 8 新增整组滞后消融 `lag_ablation_analysis`（§7.2 佐证）、`_recovery_block` 兼容嵌套 coverage 契约 + 报告测试、run_cell excluded_breakdown 统一患者口径（unknown 行级单独字段）、`patient_bootstrap_ci` 无效重采样契约（单类样本丢弃、有效 <2 → NaN，规则/模型 CI 同步）、版本残留清理。**v5.4 按 Codex 计划第八轮 10 项修正**——Task 2 schema 断言加 `unobservable_reason` + 无潜在事件（g=0）/无参考 landmark 分别处理（可转绿）、R1/R2 条件成立率按**各自指定确认 landmark** 验证（交集组 = w_A+1 双条件、补 R2-only 与 neither 首参考不命中两规则）+ 原因互斥/绑定/计数闭合 + condition_not_held 分母限定实际进入条件检查的患者、删 per_group coverage ≥0.9 门槛（与删失不设门槛一致，保留 §10 per-rule ≥0.80）、neither 误报分母全文统一（Task 3 算法说明）+ 三类患者手工 fixture（误报/有参考非误报/无参考，断言 1/3 识别分母错误）、`_discover_frozen` 去除 `_standard_candidates` 组合优先（植入语义泄漏）→ 确定性枚举 + lift 排序 + 组合预算防截断（显式 raise）、`_fold_discover_validate` 按**唯一患者**定折数（Bootstrap 重复患者不重复计数）+ 唯一正例重复确定性测试 + 收紧 CI 失败契约（"CI 未估计"确定性断言）、`lag_ablation_analysis` 消融组 = `lags` 对应滞后观测列（不误删 slope/rises/drop_pct）+ 基线/移除后均报告患者 Bootstrap CI、`reliability_boundary` 统一为规格算法（每网格点 CI 下界曲线首达 50%，boundary_events 与 point_boundary_events 分列）、lead-lag per_indicator_n 限定相关路径组 + 有匹配合格对照（§10 口径）+ 风险集匹配检查对照 index time 前删失、模拟器观测截断 `T = min(事件, 删失, 行政终点)`（§5.4 第 9 步，杜绝未来观测）。**v5.5 按 Codex 计划第九轮 6 项修正**——simulate 观测改 `patient_rows` 逐患者收集（修 `obs_rows[-n_win:]` 截断后跨患者污染，条件判定只用当前患者行）+ 观测截断无跨患者测试、`reliability_boundary` 去掉 `_point_boundary` 提前判边情形（point 仅诊断，边情形只由 CI 下界曲线判定；"原始点达标但 CI 下界跨 50%" fixture `test_boundary_point_high_ci_lower_crosses`）、`_discover_frozen` 排序改二级键 `(-lift, canonical_rule)`（并列 lift 不依赖组合枚举顺序）+ `test_discover_sorted_by_lift_then_canonical` + Task 9 Step 4 残留清理、`unobservable_reason` 逐类绑定与计数闭合测试（no_feasible_anchor⇒锚点 NaN、censored⇒confirm≥删失、event_before_confirm⇒confirm≥事件、condition_not_held⇒锚点合格事件删失在后且条件实际失败）、`lag_ablation` Bootstrap CI 可估计/不足样本两测试（(nan,nan) 必须）+ `_fmt_ci` NaN→`[NA, NA]` + 报告测试、synthetic fixture 提取 helper + `test_synthetic_fixture_r1_rule_in_top_k`（R1 canonical 规则确定性进 top_k，Task 14 依赖链有测试支撑而非仅注释）。**v5.6 按 Codex 计划第十轮 6 项修正**——synthetic fixture 加 **4 个困难负例**（R1 四条件各缺一：缺 sex/缺 age50(45 满足 age>40)/缺 HbA1c2(rises1)/缺 PLTdrop0.2(-0.15)，覆盖所有非 R1 的 1-4 条件组合）使 **R1 四条件是 lift 严格唯一最高** → 确定性进 top_k（修阻断项：7 正例条件 98 条并列时 R1 排 ~60 进不了 top20）、`_records_var` 参数改 mean=0.55/spread=0.5+clip（固定 seed=0 下 CI 下界 ≈0.47 <0.5 确跨 50%）+ 断言边界 ∈ (0,20)、`_ext_percentile` 扩展实数分位数（含 +inf 确定性处理，不用 np.percentile）+ `_fmt_ci` 渲染 `(finite, +inf)` 为 `[lo, +inf]`（不误报 [NA, NA]）、抽取 `_classify_observable` 纯函数（路径组判定收敛；`_r1_holds` 容错缺失窗口）+ 两患者手工 fixture 回归跨患者污染（`test_classify_observable_uses_current_patient_obs`，因旧缺陷不改变最终 obs 窗口集合、原截断测试无法检测）、四类原因非空手工 fixture（`test_unobservable_reason_classification_fixture` 验证优先级/绑定/互斥，修空集合通过）、`_insufficient_frame` 显式 1 正例 + 5 负例（min=1<2 确定性 not_estimable，修 `_lm().iloc[:5]` 不确定）、残留清理（config/候选注释措辞、自检标题版本）。**v5.7 按 Codex 计划第十一轮 4 项修正**——接线级回归测试 `test_simulate_classification_uses_current_patient_rows`（monkeypatch `_classify_observable` 记录 simulate **实际传入**的 by_w 键集，断言每路径组患者 by_w 只含自身观测窗口 0..min(事件,删失,admin_end) 且存在截断患者场景——仅手工调用纯函数无法捕获实现者重新使用旧全局切片接线）、`test_ext_percentile_with_inf`（扩展实数分位数确定性：中位有限、高上分位 +inf、空→nan）+ `test_fmt_ci_inf_and_nan_contract`（(finite,+inf)→"[lo, +inf]"、(nan,nan)/None→"[NA, NA]"）、`test_synthetic_fixture_r1_rule_is_unique_max_lift`（枚举全部候选组合后最大 lift **唯一且对应 R1**——困难负例覆盖论证的可执行证明）、config 测试注释措辞更新为"通用候选网格"。**v5.8 按 Codex 计划第十二轮 1 项修正**——第 12 行历史修订长句已统一改写为中性表述、清理旧措辞残留，全文通过残留检查。**v5.9 按整体复审（final review）6 项缺口修正**——`_candidate_conditions` 实现规格 §8.1 折内候选（SHAP top-M 特征 + 折内分位数阈值 + 固定临床网格补充）、`_discover_frozen` 改 **Apriori 逐层枚举 + 训练折支持度剪枝**（top_m/thresholds_per_feature 版本化，防截断 raise）、run_cell 模型改**全量 qualifying_landmarks**（unobservable 只排除确认子集流程，§5.5 角色表）、`run_method_validation` 增加**信号门槛门控**（AUC CI 下界 <0.65 → 归因与规则挖掘停止并亮红灯，`signal_gate` 返回 + acceptance 门控 + monkeypatch 测试）、`label_for` 事件==删失同窗 → **unknown**（`ev >= cw`，不得落负例 + 确定性测试）、`reliability_boundary` 保留**扩展实数 CI**（全 +inf 样本集不丢弃；boundary_samples 空 → not_estimable）+ `_scale_block` 渲染**"精度目标未达到"**（precision_not_met 列）、`_latent_risk_calibration` + `render_report` **潜在风险校准块**（目标/实际/误差 ±3pp，并入第 2 节信号验证）+ 测试。**v5.10 按整体复审二轮 6 项缺口修正**——`render_report` 第 8 节**实际渲染 limitations**（门控/失败原因可见，`_limitations_block` + `test_limitations_rendered`，修门控测试必失败）、`_candidate_conditions` 契约落实（**SHAP 输入只含可转换规则特征**（排除 admin_end/*_d6m/*_d12m/*_slope）、`_condition_from_feature` 支持 sex_male→eq、drop_pct 分位 **abs(v) 正的下降幅度**（_hits `<= -c.value` 方向正确）、候选 canonical 整体去重；删"其余指标 cur 高分位"块）+ `test_candidate_contract`（元数据排除/top-M 上限/每特征切点数/drop_pct 符号/无重复）、`_discover_frozen` **seen_rules 去重 + 预算按评估组合数**（含未通过支持度的组合，`test_discover_no_duplicate_rules` + `test_discover_budget_raises`）、`_latent_risk_calibration` 改 **独立 N=50,000 校准队列**（cal_n，种子 3）估计四组潜在风险 + **ok = 四组 |误差| ≤ 0.03（±3pp 硬断言）** + 报告校准达标/未达标状态 + `test_full_pipeline_fields` ±3pp 断言、+inf 契约**层级明确**（"全 +inf 保留 (inf,inf)" 为 `_ext_percentile` 单元级契约，`test_ext_percentile_all_inf`；reliability_boundary observed 分支边界样本必然非空含有限值，`not boundary_samples` 为防御性保护）、run_cell **model/evaluator 患者与 landmark 分列**（model_patients/model_landmarks = 全量 lm；evaluator_patients/evaluator_landmarks = 可评估口径；excluded_ratio 注释更新"未进 evaluator 流程"；`oof_events <= model_patients`）+ 测试与 Task 14 断言更新。**v5.11 按整体复审三轮 5 项修正**——report.py 补 `import config as cfg`（`_calibration_block` 用 `cfg.SIM["calibration_n"]`，修 NameError）、`_latent_risk_calibration` ok 判定**显式要求 actual 有限**（NaN 比较恒 False 会错误保留 ok=True；`_group_latent_risk` 提升为 main 模块级 import）+ `test_calibration_ok_fails_on_nan`、run_cell **evaluator 统计改从实际 confirmation_subset 输入**（`sub_eval = sub[~sub["unobservable"]]`，不再用 lm 过滤；excluded_breakdown 三分类 `unobservable + no_feasible_landmark + unknown_patients + evaluator_patients == nominal_n`，unknown_patients 来自 confirmation_subset 剔除行（每患者一行 → 患者级），qualifying_landmarks 行级 unknown 仍单独顶层字段）+ 闭环测试更新、config/fixture/自检清单 **Apriori 口径统一**（评估组合数预算、逐层支持度剪枝，无旧枚举口径残留）、`_candidate_conditions` 分位 **drop_pct 值 ≤0 过滤**（无正下降幅度不产生候选，`test_candidate_contract` 的 drop_pct >0 断言确定性成立）。**v5.12 按整体复审四轮 5 项修正**——main.py 补 `import numpy as np`（`_latent_risk_calibration` 有限性检查，修 NameError）、`_excluded_patient_sets` **患者级互斥分配**（优先级 unobservable → unknown → no_feasible → evaluator；unknown 患者 = 确认 landmark 合格且非 unobservable 且 label_for=="unknown"，与 confirmation_subset 剔除一致；unobservable 患者即使确认窗口判定 unknown 也不重复计数；闭环由集合互斥保证而非 max 偶然）+ `test_excluded_sets_mutually_exclusive_with_overlap`（unobservable+unknown 重叠场景）、Task 11 接口说明更新为**三分类闭环**（`unobservable + no_feasible_landmark + unknown_patients + evaluator_patients == nominal_n`，excluded_ratio 语义同步）、Task 13 接口签名更新 `_latent_risk_calibration(horizon_months, hw, cal)`、第 12 行历史措辞中性化（无旧枚举口径残留，且 v5.11 说明不再逐字引用）。**v5.13 按整体复审五轮 2 项修正**——第 12 行历史修订长句旧组合枚举口径全部改写为中性表述（v5.3 段"组合枚举确定性"→"发现流程确定性"；禁用措辞 grep 检查通过）、Task 11 接口 evaluator 语义明确（**evaluator = confirmation_subset 中 `~unobservable` 且 label ∈ {0,1} 的患者/landmark；unknown 患者单列为 `unknown_patients`、不进 evaluator；excluded_ratio 含 unobservable、unknown、no-feasible 三类**）。**v5.14 按整体复审六轮 1 项修正**——`_excluded_patient_sets` 互斥分配补强：**evaluator_ids = 调用者提供 ID 先与患者全集求交、再剔除 unobs/unknown**（四集合互斥不依赖调用者输入干净；后由 v5.15 显式写入 `& all_ids`）+ **确认 landmark 资格显式化**（confirm 有限且 `w ≥ 2` 且 `admin_end − w ≥ hw`，§5.5）+ `test_excluded_sets_evaluator_overlap_removed`（evaluator ID 与 unobservable/unknown 重叠场景，断言仍 1/1/1/1）。**v5.15 按整体复审七轮 1 项修正**——`_excluded_patient_sets` evaluator_ids **先 ∩ 患者全集**（`(supplied & all_ids) − unobs − unknown`，外部"幽灵"ID 不计入 n_evaluator、也不减少 no_feasible，四集合闭环不失真）+ `test_excluded_sets_ghost_eval_ids_ignored`（supplied 含不存在 ID 99 → 仍 1/1/1/1 且闭环 = 4）。**v5.16 按整体复审八轮 1 项修正**——`_excluded_patient_sets` docstring 与 v5.14 历史简写同步为"**调用者提供 ID 先与患者全集求交、再剔除 unobservable/unknown**"（消除 stale 注释，全文口径一致）。**v5.17 按整体复审九轮 1 项修正**——Task 11 重叠测试注释同步为"`evaluator_ids = (supplied ∩ 患者全集) − unobs − unknown`"（全文口径一致）。**v5.18 按整体复审十轮 1 项修正**——自检清单 v5.14 追加行历史简写同步为"`(supplied ∩ 患者全集) − unobs − unknown`"（全文无 stale 简写）。
...
- **TDD**：每步先写失败测试→红→最小实现→绿→单独提交。提交正文三行：`AI-Agent: Claude-Code`、`AI-Client: VS-Code`、`Task-ID: research-progression-min-loop`。
...
GRID = {"method_validation": {"n": 1500, "followup_months": 60, "horizon_months": 24},
        # v5.21：followup_months=24（admin_end=4）在锚点约束（w_R1 ≥ w0+3 ≥ 3 >
        # admin_end−hw=2）下路径组全 no_feasible（simulate(10k,24,12) 实测 per-rule
        # coverage 全 0）→ Task 11 实现须将该单元显式标记 not_estimable 并从恢复率/
        # 可靠性边界统计排除（规格 §5.5），不产生 coverage=0 的假"规模退化"
...
git commit -m "feat(research): 脚手架 + 版本化 config.py" -m "AI-Agent: Claude-Code
AI-Client: VS-Code
...
def _conditions_hold_at_anchor(out, rows, anchor_fn, horizon_windows, which):
    ...
    噪声下两窗连升概率 = 1/6 ≈ 16.7%（三个 iid 连续值严格递增的排列概率，与 σ 无关；
    v5.20 误写 25%，实测 17.5% 吻合 1/6），其命中如实计入
...
def simulate(n, followup_months, horizon_months, seed, gate=None, _lambda_c=None):
    ...
    # 与逐指标定义一致；v5.20 曾误用 0.1·范围 → PLT 基线 SD 37.5→22.5，v5.21 修复）
    inds = cfg.INDICATORS
    n_ind = len(inds)
...
git commit -m "feat(research): 模拟核心（前向生成 + 完整不可观测判定 + 组语义保留）" -m "AI-Agent: Claude-Code
AI-Client: VS-Code
Task-ID: research-progression-min-loop"
```
...
def test_calibrated_latent_risk_both_horizons():
    from simulate_cohort import _group_latent_risk   # v5.20：计划测试代码原缺 import（14 轮评审未运行测试未暴露）
...
def _patient_order(pids):
    ...
    一次 O(N log N)，调用方**每个 obs 集只构建一次**并复用（勿在患者循环内重复排序）；
    _by_w_slice 不再假设全局排序（v5.21：行偏移推导误判；v5.22：searchsorted 全局
    有序假设在完整 DataFrame shuffle 后误判 neither 误报率 0.152→0.001）。"""
...
def _by_w_slice(sorted_pids, order, wins, vals, cols, pid, w):
    ...
    按稳定排序索引定位患者块（`order` 由 _patient_order 构建，**每 obs 集只构建一次**），
    块内**按实际 window 列匹配**：全局乱序/块内乱序/缺窗/非零起始窗口均正确，
    与输入行序无关（v5.22，Codex 二轮 P1-2）。
...
def _group_latent_risk(out, grp, hw, obs=None):
    ...
    **校准分母** = 有合格参考 landmark 且非误报的 neither 患者（规格 §5.3 明确定义；
    ...
    if grp == "neither":
        ...
        order = _patient_order(pids)                 # 每 obs 集只构建一次（v5.22）
...
实现说明：`simulate` 的 coverage 已在 Task 2 收尾段填实（v5.20 向量化收尾直接调用 `_compute_coverage(patients, obs_df, meta)`）。`per_group`/`per_rule` 分母 = 该组患者总数（含 unobservable）、分子 = 可评估患者，与 §10 覆盖率门槛口径一致；`excluded` 单列。
...
git commit -m "feat(research): bisection 校准（可缩放 λ_b）+ P_obs + coverage 具体算法" -m "AI-Agent: Claude-Code
AI-Client: VS-Code
Task-ID: research-progression-min-loop"
```
...
def test_qualifying_uses_all_landmarks():
    ...
    # 至多 1 窗、len(lm) > nunique 数学上不可能（v5.25，v5.24 锚点契约连锁，同 coverage 测试）
...
def test_empty_landmarks_preserve_schema():
    """空数据集保留完整固定列 schema 与 dtype（v5.26/27，Codex 批次 2 P1）：下游
...
git commit -m "feat(research): 窗口特征 + 分角色 landmark + sex 编码 + 标签" -m "AI-Agent: Claude-Code
AI-Client: VS-Code
Task-ID: research-progression-min-loop"
```
...
git commit -m "feat(research): 患者折 + 聚类 Bootstrap（保留 multiplicity）" -m "AI-Agent: Claude-Code
AI-Client: VS-Code
Task-ID: research-progression-min-loop"
```
...
def _lm():
    ...
    # event_pat=0 → not_estimable（v5.25，v5.24 锚点契约连锁，同 coverage/features 测试修正）
...
def test_n_repeats_must_match_seeds():
    """n_repeats 与 seeds 长度显式校验（v5.26，Codex 批次 2 P2-1）：不一致/零重复
...
def test_model_features_exclude_admin_end():
    """模型特征 = 指标派生 + age + sex_male，**不含 admin_end**（v5.26，Codex 批次 2
    P2-2：随访设计/资格元数据不进模型，避免合并队列时泄漏研究设计差异）。"""
    from model import _feat_cols
    lm = _lm()
    cols = _feat_cols(lm)
...
def test_empty_landmarks_not_estimable_chain():
    ...
    空 oof_frame 保留 patient_id/label/oof schema（v5.26/27，Task 11 直接读
    oof_frame["label"]，原无列 DataFrame 会 KeyError）。"""
    lm = _lm().iloc[0:0]
...
git commit -m "feat(research): 患者级 OOF（患者聚合分层 + 全数值特征）" -m "AI-Agent: Claude-Code
AI-Client: VS-Code
Task-ID: research-progression-min-loop"
```
...
git commit -m "feat(research): lead-lag（观察进展者 + 嵌套结构 + 真实破平 + CI）" -m "AI-Agent: Claude-Code
AI-Client: VS-Code
Task-ID: research-progression-min-loop"
```
...
git commit -m "feat(research): 规则挖掘（标准词汇 + 完整 CI 重跑 + 数值规范化）" -m "AI-Agent: Claude-Code
AI-Client: VS-Code
Task-ID: research-progression-min-loop"
```
...
def evaluate(recovery, planted_rules, subset, coverage):
    ...
    return {
        ...
        # 杂项规则的 CI 未估计是统计现实，不作为 acceptance 门控）
...
git commit -m "feat(research): evaluator（类型化命中含 horizon/lookback/lag + 实例级绑定 R1/R2 规则）" -m "AI-Agent: Claude-Code
AI-Client: VS-Code
Task-ID: research-progression-min-loop"
```
...
def test_run_study_excludes_infeasible_unit():
    ...
    assert cell36.get("not_estimable") is False and "overall_mean" in cell36
    assert out["reliability_boundaries"]["f24"].get("status") == "not_estimable"
    assert out["reliability_boundaries"]["f36"].get("status") != "not_estimable"
...
git commit -m "feat(research): 规模退化 Monte Carlo + 可靠性边界（曲线 CI 下界 + 边情形）" -m "AI-Agent: Claude-Code
AI-Client: VS-Code
Task-ID: research-progression-min-loop"
```
...
git commit -m "feat(research): CLI 完整数据流 + README + 数据流签名断言" -m "AI-Agent: Claude-Code
AI-Client: VS-Code
Task-ID: research-progression-min-loop"
```
...
git commit -m "feat(research): 端到端分层测试（slow/acceptance/现实规模参数化/可复现）" -m "AI-Agent: Claude-Code
AI-Client: VS-Code
Task-ID: research-progression-min-loop"
```
...
- **v5.14 追加（整体复审六轮）**：`_excluded_patient_sets` evaluator 显式剔除（(supplied ∩ 患者全集) − unobs − unknown）+ 确认 landmark 资格显式化（w ≥ 2 且 admin_end − w ≥ hw）+ evaluator 重叠测试。
- **v5.15 追加（整体复审七轮）**：evaluator_ids 先 ∩ 患者全集（幽灵 ID 忽略）+ 测试。
- **v5.16 追加（整体复审八轮）**：`_excluded_patient_sets` docstring 与 v5.14 历史简写同步（求交后再剔除，消除 stale 注释）。
- **v5.17 追加（整体复审九轮）**：Task 11 重叠测试注释同步（supplied ∩ 患者全集 − unobs − unknown）。
- **v5.18 追加（整体复审十轮）**：自检清单 v5.14 追加行历史简写同步（全文无 stale 简写）。
- **无占位符**：所有任务（含 Task 4/5/6）均含实际测试代码与可执行实现；无"同 vN"或"实现者须补齐"。

## 执行交接

计划已保存（v5.18）。执行选项：**Subagent-Driven（推荐）** 或 **Inline Execution**。按既定协作框架建议**分批执行（Task 1-3、4-6、7-10、11-14）、每批交 Codex 审查、通过后推送**。实施计划 v5.18 先交 Codex 复审。
