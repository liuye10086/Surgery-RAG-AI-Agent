...
def _excluded_patient_sets(patients, supplied_eval_ids, hw):
    ...
    - **evaluator_ids = 调用者提供的 ID − unobs_ids − unknown_ids**（显式剔除——即使调用者传入
      含 unobservable/unknown 患者的 ID，四集合仍互斥，不依赖调用者输入干净）；
    - no_feasible_ids = 剩余。
...
计划已保存（v5.15）。执行选项：**Subagent-Driven（推荐）** 或 **Inline Execution**。按既定协作框架建议**分批执行（Task 1-3、4-6、7-10、11-14）、每批交 Codex 审查、通过后推送**。实施计划 v5.15 先交 Codex 复审。
